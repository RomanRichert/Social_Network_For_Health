...
public class HealthScoreCalculator {
    ...
    private Map<Integer, Integer> sf36Answers = new LinkedHashMap<>();

    static {}



    private List<Integer> pf = List.of(
            sf36Answers.get(3),
            sf36Answers.get(4),
            sf36Answers.get(5),
            sf36Answers.get(6),
    ...
    private List<Integer> rp = List.of(
            sf36Answers.get(13),
            sf36Answers.get(14),
            sf36Answers.get(15),
            sf36Answers.get(16)
    ...
    private List<Integer> bp = List.of(
            sf36Answers.get(21),
            sf36Answers.get(22)
    );

    private List<Integer> gh = List.of(
            sf36Answers.get(1),
            sf36Answers.get(33),
            sf36Answers.get(34),
            sf36Answers.get(35),
    ...
    private List<Integer> vt = List.of(
            sf36Answers.get(23),
            sf36Answers.get(27),
            sf36Answers.get(29),
            sf36Answers.get(31)
    ...
    private List<Integer> sf = List.of(
            sf36Answers.get(20),
            sf36Answers.get(32)
    );

    private List<Integer> re = List.of(
            sf36Answers.get(17),
            sf36Answers.get(18),
            sf36Answers.get(19)
    );

    private List<Integer> mh = List.of(
            sf36Answers.get(24),
            sf36Answers.get(25),
            sf36Answers.get(26),
            sf36Answers.get(28),
    ...
    public static double calculateHealthScore(StoryRequestDTO story){
        Map<Integer, Integer> map = story.getSf36Answers();
        sf36Answers.clear();
        sf36Answers.putAll(map);

        return (calculateMentalHealth() + calculatePhysicalHealth()) / 2;
    }

    private static double calculateMentalHealth(){
        return (pfz() * -0.22999) + (rpz() * -0.12329) + (bpz() * -0.09731) + (getSocialFunctioning() * 0.26876) + (mhz()
                * 0.48581) + (rez() * 0.43407) + (vtz() * 0.23534) + (ghz() * -0.01571);
    }

    private static double calculatePhysicalHealth() {
        return (((pfz() * 0.42402) + (rpz() * 0.35119) + (bpz() * 0.31754) + (sfz() * -0.00753) + (mhz()
                * -0.22069) + (rez() * -0.19206) + (vtz() * 0.02877) + (ghz() * 0.24954)) * 10) + 50;
    }

    private static double ghz() {
        double ghSum;
        double gh1 = 0.0;
    ...
    private static double sfz() {
        return (getSocialFunctioning() - 83.59753) / 22.37642;
    }

    private static double getSocialFunctioning(){
        double sf6 = 0.0;
